...
class KnowledgeNeurons:
    ...
    def _generate(self, prompt, ground_truth):
        ...
        n_sampling_steps = len(target_label) if ("qwen" in self.model_type or "gpt" in self.model_type or 'llama' in self.model_type or 'chatglm' in self.model_type) else 1
        all_gt_probs = []
        all_argmax_probs = []
        argmax_tokens = []
    ...
    def modify_weights(
        ...
        if mode == "edit":
            ...
            if self.model_type == "gpt2" or self.model_type == "chatglm":
